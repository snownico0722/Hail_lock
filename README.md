...
> [!NOTE]
> 开启此限制后，即使用户处于未冻结状态，也无法直接在系统桌面或设置中卸载该软件。若需解除禁止卸载限制，只需将命令中倒数第二个参数（即控制 `blockUninstall` 的布尔值）由 `i32 1` 更改为 `i32 0` 并重新执行即可（例如在 Android 14 上解除限制：`adb shell service call package 134 s16 包名 i32 0 i32 0`）。

#### 与“禁止安装和更新应用（全局）”的区别

上面的 ADB `blockUninstall` 方法是**按应用生效**的：只阻止指定目标应用被卸载，不影响您安装新应用，也不影响其他应用正常更新。对于只想保护少数目标应用、防止通过“卸载后重装”绕过限制的场景，这种方式通常更灵活。

Hail 设置中的“禁止安装和更新应用（全局）”则是更强的设备级限制：开启后整个设备都会被限制安装和更新应用，防绕过能力更强，但日常安装新软件、更新应用和维护设备也会更麻烦。因此它不是按应用禁止卸载的替代品，两种方式可以按需求单独使用，也可以组合使用。

Android 的 Device Owner 还提供正式的按应用禁止卸载 API `DevicePolicyManager.setUninstallBlocked()`。后续可以在 Hail 内补充对应的图形界面；即使增加 UI，上面的 ADB 方法仍会保留，作为无需额外界面操作的独立方案。
